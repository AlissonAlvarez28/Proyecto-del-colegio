...
(function () {
  ...
  function dibujarBoton() {
    var cont = document.getElementById('sesion-widget');
    if (!cont) return;
    var u = Sesion.usuario();

    if (Sesion.activa() && u) {
      var enlacePanel = Sesion.esColaboradorOAdmin()
      ...
      cont.innerHTML =
        ...
        enlacePanel +
        '<button type="button" class="btn btn-outline-light btn-sm" id="btn-cerrar-sesion-publico">Salir</button>';
      document.getElementById('btn-cerrar-sesion-publico').onclick = Sesion.cerrar;
    } else {
      cont.innerHTML = '<button type="button" class="btn btn-outline-light btn-sm" id="btn-abrir-sesion">' +
        '<i class="fas fa-right-to-bracket" aria-hidden="true"></i> Iniciar sesión</button>';
      document.getElementById('btn-abrir-sesion').onclick = function () { abrirModal('login'); };
    }
  }
